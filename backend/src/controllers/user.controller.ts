...
export async function register(req: Request, res: Response) {
  ...
  try {
    ...
    return res.status(201).json(out); // { id, email, name, token }
  } catch (e: any) {
    const code = e?.message;

    if (code === "invalid_email")   return res.status(400).json({ error: "Email inválido" });
    if (code === "weak_password")   return res.status(400).json({ error: "Senha fraca" });
    if (code === "invalid_name")    return res.status(400).json({ error: "Nome inválido" });
...
export async function login(req: Request, res: Response) {
  ...
  try {
    const out = await loginUser(email, password); // { token }
